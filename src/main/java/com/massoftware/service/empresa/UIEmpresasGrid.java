...
public class UIEmpresasGrid extends GridCustom<Empresas> {

	private EmpresasFiltro filter;
	private EmpresaService service;

	public UIEmpresasGrid(EmpresaService service, EmpresasFiltro filter) {
		super(Empresas.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(Empresas::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(Empresas::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Empresa")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<Empresas> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<Empresas> items = service.find(filter);
	...
	protected boolean removeItemFromService(Empresas item) {
