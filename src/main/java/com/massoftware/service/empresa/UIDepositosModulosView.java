...
@PageTitle("Módulos")
@Route("DepositosModulos")
public class UIDepositosModulosView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private DepositosModulosFiltro lastFilter;
	private DepositosModulosFiltro filter;
	private Binder<DepositosModulosFiltro> binder;
	...
	// Grid
	private UIDepositosModulosGrid grid;

	public UIDepositosModulosView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new DepositosModulosFiltro();
		binder = new Binder<>(DepositosModulosFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		binder.forField(numeroFrom)
			...
			.bind(DepositosModulosFiltro::getNumeroFrom, DepositosModulosFiltro::setNumeroFrom);
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(DepositosModulosFiltro::getNumeroTo, DepositosModulosFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(DepositosModulosFiltro::getNombre, DepositosModulosFiltro::setNombre);
...
//		grid = new UIDepositosModulosGrid(AppCX.services().buildDepositoModuloService(), filter);
		grid = new UIDepositosModulosGrid(new DepositoModuloService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (DepositosModulosFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
