...
public class UIDepositosModulosGrid extends GridCustom<DepositosModulos> {

	private DepositosModulosFiltro filter;
	private DepositoModuloService service;

	public UIDepositosModulosGrid(DepositoModuloService service, DepositosModulosFiltro filter) {
		super(DepositosModulos.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(DepositosModulos::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(DepositosModulos::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Módulo")
			.setVisible(false);
		...
		addColumn(DepositosModulos::getNumero, "numero")
			.setKey("numero")
			.setResizable(true)
			.setSortProperty("2")
			.setHeader("Nº módulo");

		addColumn(DepositosModulos::getNombre, "nombre")
			.setKey("nombre")
			.setResizable(true)
			.setSortProperty("3")
			.setHeader("Nombre");
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<DepositosModulos> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<DepositosModulos> items = service.find(filter);
		...
		return new ArrayList<DepositosModulos>();
	}

	protected boolean removeItemFromService(DepositosModulos item) {
