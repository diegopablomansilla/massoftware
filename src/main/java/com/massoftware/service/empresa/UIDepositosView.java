...
@PageTitle("Depósitos")
@Route("Depositos")
public class UIDepositosView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private DepositosFiltro lastFilter;
	private DepositosFiltro filter;
	private Binder<DepositosFiltro> binder;
	...
	// Grid
	private UIDepositosGrid grid;

	public UIDepositosView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();
	...
	private void buildBinder() {
		filter = new DepositosFiltro();
		binder = new Binder<>(DepositosFiltro.class);
	...
	private void buildFilterRows() {
		...
		binder.forField(numeroFrom)
			...
			.bind(DepositosFiltro::getNumeroFrom, DepositosFiltro::setNumeroFrom);
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(DepositosFiltro::getNumeroTo, DepositosFiltro::setNumeroTo);
		...
		binder.bind(nombre, DepositosFiltro::getNombre, DepositosFiltro::setNombre);
...
*/
	...
	private void buildGrid() throws Exception {
		grid = new UIDepositosGrid(AppCX.services().buildDepositoService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (DepositosFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
