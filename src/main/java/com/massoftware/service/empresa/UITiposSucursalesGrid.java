...
public class UITiposSucursalesGrid extends GridCustom<TiposSucursales> {

	private TiposSucursalesFiltro filter;
	private TipoSucursalService service;

	public UITiposSucursalesGrid(TipoSucursalService service, TiposSucursalesFiltro filter) {
		super(TiposSucursales.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(TiposSucursales::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(TiposSucursales::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Tipo de sucursal")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<TiposSucursales> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<TiposSucursales> items = service.find(filter);
		...
		return new ArrayList<TiposSucursales>();
	}

	protected boolean removeItemFromService(TiposSucursales item) {
