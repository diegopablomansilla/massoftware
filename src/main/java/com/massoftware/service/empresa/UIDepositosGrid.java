...
public class UIDepositosGrid extends GridCustom<Depositos> {

	private DepositosFiltro filter;
	private DepositoService service;

	public UIDepositosGrid(DepositoService service, DepositosFiltro filter) {
	...
	protected void addColumns() {
		...
		addColumn(Depositos::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(Depositos::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Depósito")
			.setVisible(false);
	...
	*/
	...
	protected List<Depositos> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<Depositos> items = service.find(filter);
	...
	protected boolean removeItemFromService(Depositos item) {
