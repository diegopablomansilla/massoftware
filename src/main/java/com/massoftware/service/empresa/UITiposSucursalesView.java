...
@Route("TiposSucursales")
public class UITiposSucursalesView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private TiposSucursalesFiltro lastFilter;
	private TiposSucursalesFiltro filter;
	private Binder<TiposSucursalesFiltro> binder;
	...
	// Grid
	private UITiposSucursalesGrid grid;

	public UITiposSucursalesView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new TiposSucursalesFiltro();
		binder = new Binder<>(TiposSucursalesFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		numeroFrom.setPlaceholder("Nº tipo de sucursal desde ");
		...
		binder.forField(numeroFrom)
			...
			.bind(TiposSucursalesFiltro::getNumeroFrom, TiposSucursalesFiltro::setNumeroFrom);
		...
		numeroTo.setPlaceholder("Nº tipo de sucursal hasta ");
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(TiposSucursalesFiltro::getNumeroTo, TiposSucursalesFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(TiposSucursalesFiltro::getNombre, TiposSucursalesFiltro::setNombre);
...
//		grid = new UITiposSucursalesGrid(AppCX.services().buildTipoSucursalService(), filter);
		grid = new UITiposSucursalesGrid(new TipoSucursalService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (TiposSucursalesFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
