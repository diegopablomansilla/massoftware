...
@PageTitle("Monedas AFIP")
@Route("MonedasAFIP")
public class UIMonedasAFIPView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private MonedasAFIPFiltro lastFilter;
	private MonedasAFIPFiltro filter;
	private Binder<MonedasAFIPFiltro> binder;
	...
	// Grid
	private UIMonedasAFIPGrid grid;

	public UIMonedasAFIPView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new MonedasAFIPFiltro();
		binder = new Binder<>(MonedasAFIPFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		binder.forField(codigo)
			.bind(MonedasAFIPFiltro::getCodigo, MonedasAFIPFiltro::setCodigo);
		...
		binder.forField(nombre)
			.bind(MonedasAFIPFiltro::getNombre, MonedasAFIPFiltro::setNombre);
...
//		grid = new UIMonedasAFIPGrid(AppCX.services().buildMonedaAFIPService(), filter);
		grid = new UIMonedasAFIPGrid(new MonedaAFIPService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (MonedasAFIPFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
