...
@PageTitle("Tipos de documentos")
@Route("TiposDocumentosAFIP")
public class UITiposDocumentosAFIPView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private TiposDocumentosAFIPFiltro lastFilter;
	private TiposDocumentosAFIPFiltro filter;
	private Binder<TiposDocumentosAFIPFiltro> binder;
	...
	// Grid
	private UITiposDocumentosAFIPGrid grid;

	public UITiposDocumentosAFIPView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new TiposDocumentosAFIPFiltro();
		binder = new Binder<>(TiposDocumentosAFIPFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		binder.forField(numeroFrom)
			...
			.bind(TiposDocumentosAFIPFiltro::getNumeroFrom, TiposDocumentosAFIPFiltro::setNumeroFrom);
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(TiposDocumentosAFIPFiltro::getNumeroTo, TiposDocumentosAFIPFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(TiposDocumentosAFIPFiltro::getNombre, TiposDocumentosAFIPFiltro::setNombre);
...
//		grid = new UITiposDocumentosAFIPGrid(AppCX.services().buildTipoDocumentoAFIPService(), filter);
		grid = new UITiposDocumentosAFIPGrid(new TipoDocumentoAFIPService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (TiposDocumentosAFIPFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
