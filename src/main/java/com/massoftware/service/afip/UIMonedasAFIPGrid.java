...
public class UIMonedasAFIPGrid extends GridCustom<MonedasAFIP> {

	private MonedasAFIPFiltro filter;
	private MonedaAFIPService service;

	public UIMonedasAFIPGrid(MonedaAFIPService service, MonedasAFIPFiltro filter) {
		super(MonedasAFIP.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(MonedasAFIP::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(MonedasAFIP::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Moneda AFIP")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<MonedasAFIP> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<MonedasAFIP> items = service.find(filter);
		...
		return new ArrayList<MonedasAFIP>();
	}

	protected boolean removeItemFromService(MonedasAFIP item) {
