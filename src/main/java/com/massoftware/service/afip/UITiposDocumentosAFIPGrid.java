...
public class UITiposDocumentosAFIPGrid extends GridCustom<TiposDocumentosAFIP> {

	private TiposDocumentosAFIPFiltro filter;
	private TipoDocumentoAFIPService service;

	public UITiposDocumentosAFIPGrid(TipoDocumentoAFIPService service, TiposDocumentosAFIPFiltro filter) {
		super(TiposDocumentosAFIP.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(TiposDocumentosAFIP::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(TiposDocumentosAFIP::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Tipo de documento")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<TiposDocumentosAFIP> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<TiposDocumentosAFIP> items = service.find(filter);
	...
	protected boolean removeItemFromService(TiposDocumentosAFIP item) {
