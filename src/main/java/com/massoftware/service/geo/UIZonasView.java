...
@PageTitle("Zonas")
@Route("Zonas")
public class UIZonasView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private ZonasFiltro lastFilter;
	private ZonasFiltro filter;
	private Binder<ZonasFiltro> binder;
	...
	// Grid
	private UIZonasGrid grid;

	public UIZonasView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new ZonasFiltro();
		binder = new Binder<>(ZonasFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		binder.forField(codigo)
			.bind(ZonasFiltro::getCodigo, ZonasFiltro::setCodigo);
		...
		binder.forField(nombre)
			.bind(ZonasFiltro::getNombre, ZonasFiltro::setNombre);
...
//		grid = new UIZonasGrid(AppCX.services().buildZonaService(), filter);
		grid = new UIZonasGrid(new ZonaService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (ZonasFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
