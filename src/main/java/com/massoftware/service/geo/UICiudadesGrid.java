...
public class UICiudadesGrid extends GridCustom<Ciudades> {
	...
	public UICiudadesGrid(CiudadService service, CiudadesFiltro filter) {
	...
	protected void addColumns() {
		...
		addColumn(Ciudades::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(Ciudades::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Ciudad")
			.setVisible(false);
	...
	*/
	...
	protected List<Ciudades> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<Ciudades> items = service.find(filter);
	...
	protected boolean removeItemFromService(Ciudades item) {
