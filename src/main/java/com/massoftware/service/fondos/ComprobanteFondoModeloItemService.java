...
public class ComprobanteFondoModeloItemService {
	...
	public List<ComprobanteFondoModeloItem> find() throws Exception {		

		DataBase db = BackendContextPG.get().getDataBase();

		List<ComprobanteFondoModeloItem> listado = null;

		try {

			db.begint();

			listado = db.fillAllObjects(ComprobanteFondoModeloItem.class);
		...
		if (listado == null) {
			listado = new ArrayList<ComprobanteFondoModeloItem>();
		}
