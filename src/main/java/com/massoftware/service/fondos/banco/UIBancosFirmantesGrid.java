...
public class UIBancosFirmantesGrid extends GridCustom<BancosFirmantes> {

	private BancosFirmantesFiltro filter;
	private BancoFirmanteService service;

	public UIBancosFirmantesGrid(BancoFirmanteService service, BancosFirmantesFiltro filter) {
		super(BancosFirmantes.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(BancosFirmantes::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(BancosFirmantes::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Firmante")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<BancosFirmantes> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<BancosFirmantes> items = service.find(filter);
		...
		return new ArrayList<BancosFirmantes>();
	}

	protected boolean removeItemFromService(BancosFirmantes item) {
