...
@PageTitle("Firmantes")
@Route("BancosFirmantes")
public class UIBancosFirmantesView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private BancosFirmantesFiltro lastFilter;
	private BancosFirmantesFiltro filter;
	private Binder<BancosFirmantesFiltro> binder;
	...
	// Grid
	private UIBancosFirmantesGrid grid;

	public UIBancosFirmantesView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new BancosFirmantesFiltro();
		binder = new Binder<>(BancosFirmantesFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		numeroFrom.setPlaceholder("Nº firmante desde ");
		...
		binder.forField(numeroFrom)
			...
			.bind(BancosFirmantesFiltro::getNumeroFrom, BancosFirmantesFiltro::setNumeroFrom);
		...
		numeroTo.setPlaceholder("Nº firmante hasta ");
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(BancosFirmantesFiltro::getNumeroTo, BancosFirmantesFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(BancosFirmantesFiltro::getNombre, BancosFirmantesFiltro::setNombre);
...
//		grid = new UIBancosFirmantesGrid(AppCX.services().buildBancoFirmanteService(), filter);
		grid = new UIBancosFirmantesGrid(new BancoFirmanteService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (BancosFirmantesFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
