...
@PageTitle("Firmante")
@Route("BancoFirmante")
public class UIBancoFirmanteView extends VerticalLayout implements HasUrlParameter<String> {

	private BancoFirmanteService service;		

	// Binder
	private BancoFirmante item;
	private Binder<BancoFirmante> binder;
	...
	public UIBancoFirmanteView() throws Exception {
		service = new BancoFirmanteService();		
		buildBinder();
		buildForm();
		this.setHeightFull();
	}
	...
	private void buildBinder() {
		item = new BancoFirmante();
		binder = new Binder<>(BancoFirmante.class);
		binder.setBean(item);
	}
	...
	private void buildNombre() throws Exception {
		...
		binder.forField(nombre)
			...
			.bind(BancoFirmante::getNombre, BancoFirmante::setNombre);
	}

	private void buildCargo() throws Exception {
		// Cargo
		cargo = new TextField();
		cargo.setLabel("Cargo");
		cargo.setWidthFull();
		cargo.setClearButtonVisible(true);
		cargo.setAutoselect(true);
		binder.forField(cargo)
			.withValidator(value -> (value != null) ? value.length() <= 50 : true, "El valor tiene que contener menos de 50 caracteres")
			.bind(BancoFirmante::getCargo, BancoFirmante::setCargo);
	}
	...
	private void buildBloqueado() throws Exception {
		...
		binder.forField(bloqueado)
			.bind(BancoFirmante::getBloqueado, BancoFirmante::setBloqueado);
	}
	...
	public void search(String id) {
		...
		try {
			...
			if (binder.isValid()) {											
				...
				BinderValidationStatus<BancoFirmante> validate = binder.validate();
