...
@PageTitle("Clases")
@Route("ClasesComprobantes")
public class UIClasesComprobantesView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private ClasesComprobantesFiltro lastFilter;
	private ClasesComprobantesFiltro filter;
	private Binder<ClasesComprobantesFiltro> binder;
	...
	// Grid
	private UIClasesComprobantesGrid grid;

	public UIClasesComprobantesView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new ClasesComprobantesFiltro();
		binder = new Binder<>(ClasesComprobantesFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		numeroFrom.setPlaceholder("Nº clase desde ");
		...
		binder.forField(numeroFrom)
			...
			.bind(ClasesComprobantesFiltro::getNumeroFrom, ClasesComprobantesFiltro::setNumeroFrom);
		...
		numeroTo.setPlaceholder("Nº clase hasta ");
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(ClasesComprobantesFiltro::getNumeroTo, ClasesComprobantesFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(ClasesComprobantesFiltro::getNombre, ClasesComprobantesFiltro::setNombre);
...
//		grid = new UIClasesComprobantesGrid(AppCX.services().buildClaseComprobanteService(), filter);
		grid = new UIClasesComprobantesGrid(new ClaseComprobanteService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (ClasesComprobantesFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
