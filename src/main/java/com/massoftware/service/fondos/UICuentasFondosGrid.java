...
public class UICuentasFondosGrid extends GridCustom<CuentasFondos> {

	private CuentasFondosFiltro filter;
	private CuentaFondoService service;

	public UICuentasFondosGrid(CuentaFondoService service, CuentasFondosFiltro filter) {
		super(CuentasFondos.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(CuentasFondos::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(CuentasFondos::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Cuenta fondo")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<CuentasFondos> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<CuentasFondos> items = service.find(filter);
		...
		return new ArrayList<CuentasFondos>();
	}

	protected boolean removeItemFromService(CuentasFondos item) {
