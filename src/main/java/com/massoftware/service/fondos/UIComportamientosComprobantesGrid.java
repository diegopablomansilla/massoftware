...
public class UIComportamientosComprobantesGrid extends GridCustom<ComportamientosComprobantes> {

	private ComportamientosComprobantesFiltro filter;
	private ComportamientoComprobanteService service;

	public UIComportamientosComprobantesGrid(ComportamientoComprobanteService service, ComportamientosComprobantesFiltro filter) {
	...
	protected void addColumns() {
		...
		addColumn(ComportamientosComprobantes::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(ComportamientosComprobantes::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Comportamiento de comprobante")
			.setVisible(false);
	...
	*/
	...
	protected List<ComportamientosComprobantes> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<ComportamientosComprobantes> items = service.find(filter);
		...
		return new ArrayList<ComportamientosComprobantes>();
	}

	protected boolean removeItemFromService(ComportamientosComprobantes item) {
