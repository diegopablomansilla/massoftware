...
public class UITicketsGrid extends GridCustom<Tickets> {

	private TicketsFiltro filter;
	private TicketService service;

	public UITicketsGrid(TicketService service, TicketsFiltro filter) {
		super(Tickets.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(Tickets::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(Tickets::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Ticket")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<Tickets> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<Tickets> items = service.find(filter);
	...
	protected boolean removeItemFromService(Tickets item) {
