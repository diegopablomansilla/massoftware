...
@PageTitle("Tipos de copias")
@Route("TiposComprobantesCopiasAlternativos")
public class UITiposComprobantesCopiasAlternativosView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private TiposComprobantesCopiasAlternativosFiltro lastFilter;
	private TiposComprobantesCopiasAlternativosFiltro filter;
	private Binder<TiposComprobantesCopiasAlternativosFiltro> binder;
	...
	// Grid
	private UITiposComprobantesCopiasAlternativosGrid grid;

	public UITiposComprobantesCopiasAlternativosView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new TiposComprobantesCopiasAlternativosFiltro();
		binder = new Binder<>(TiposComprobantesCopiasAlternativosFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		numeroFrom.setPlaceholder("Nº tipo desde ");
		...
		binder.forField(numeroFrom)
			...
			.bind(TiposComprobantesCopiasAlternativosFiltro::getNumeroFrom, TiposComprobantesCopiasAlternativosFiltro::setNumeroFrom);
		...
		numeroTo.setPlaceholder("Nº tipo hasta ");
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(TiposComprobantesCopiasAlternativosFiltro::getNumeroTo, TiposComprobantesCopiasAlternativosFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(TiposComprobantesCopiasAlternativosFiltro::getNombre, TiposComprobantesCopiasAlternativosFiltro::setNombre);
...
//		grid = new UITiposComprobantesCopiasAlternativosGrid(AppCX.services().buildTipoComprobanteCopiaAlternativoService(), filter);
		grid = new UITiposComprobantesCopiasAlternativosGrid(new TipoComprobanteCopiaAlternativoService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (TiposComprobantesCopiasAlternativosFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
