...
public class UITicketsModelosView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private TicketsModelosFiltro lastFilter;
	private TicketsModelosFiltro filter;
	private Binder<TicketsModelosFiltro> binder;
	...
	// Grid
	private UITicketsModelosGrid grid;

	public UITicketsModelosView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new TicketsModelosFiltro();
		binder = new Binder<>(TicketsModelosFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		binder.forField(numeroFrom)
			...
			.bind(TicketsModelosFiltro::getNumeroFrom, TicketsModelosFiltro::setNumeroFrom);
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(TicketsModelosFiltro::getNumeroTo, TicketsModelosFiltro::setNumeroTo);
		...
		List<Tickets> ticketItems = ticketService.find(ticketFiltro);
