...
@PageTitle("Tipos de cuenta fondo")
@Route("CuentasFondosTipos")
public class UICuentasFondosTiposView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private CuentasFondosTiposFiltro lastFilter;
	private CuentasFondosTiposFiltro filter;
	private Binder<CuentasFondosTiposFiltro> binder;
	...
	// Grid
	private UICuentasFondosTiposGrid grid;

	public UICuentasFondosTiposView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new CuentasFondosTiposFiltro();
		binder = new Binder<>(CuentasFondosTiposFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		binder.forField(numeroFrom)
			...
			.bind(CuentasFondosTiposFiltro::getNumeroFrom, CuentasFondosTiposFiltro::setNumeroFrom);
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(CuentasFondosTiposFiltro::getNumeroTo, CuentasFondosTiposFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(CuentasFondosTiposFiltro::getNombre, CuentasFondosTiposFiltro::setNombre);
...
//		grid = new UICuentasFondosTiposGrid(AppCX.services().buildCuentaFondoTipoService(), filter);
		grid = new UICuentasFondosTiposGrid(new CuentaFondoTipoService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (CuentasFondosTiposFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
