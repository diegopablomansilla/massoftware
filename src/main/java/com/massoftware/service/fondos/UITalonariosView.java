...
@PageTitle("Talonarios")
@Route("Talonarios")
public class UITalonariosView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private TalonariosFiltro lastFilter;
	private TalonariosFiltro filter;
	private Binder<TalonariosFiltro> binder;
	...
	// Grid
	private UITalonariosGrid grid;

	public UITalonariosView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new TalonariosFiltro();
		binder = new Binder<>(TalonariosFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		numeroFrom.setPlaceholder("Nº talonario desde ");
		...
		binder.forField(numeroFrom)
			...
			.bind(TalonariosFiltro::getNumeroFrom, TalonariosFiltro::setNumeroFrom);
		...
		numeroTo.setPlaceholder("Nº talonario hasta ");
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(TalonariosFiltro::getNumeroTo, TalonariosFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(TalonariosFiltro::getNombre, TalonariosFiltro::setNombre);
...
//		grid = new UITalonariosGrid(AppCX.services().buildTalonarioService(), filter);
		grid = new UITalonariosGrid(new TalonarioService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (TalonariosFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
