...
@PageTitle("Cajas")
@Route("Cajas")
public class UICajasView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private CajasFiltro lastFilter;
	private CajasFiltro filter;
	private Binder<CajasFiltro> binder;
	...
	// Grid
	private UICajasGrid grid;

	public UICajasView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new CajasFiltro();
		binder = new Binder<>(CajasFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		numeroFrom.setPlaceholder("Nº caja desde ");
		...
		binder.forField(numeroFrom)
			...
			.bind(CajasFiltro::getNumeroFrom, CajasFiltro::setNumeroFrom);
		...
		numeroTo.setPlaceholder("Nº caja hasta ");
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(CajasFiltro::getNumeroTo, CajasFiltro::setNumeroTo);
