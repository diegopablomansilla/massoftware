...
public class UITiposComprobantesConceptosGrid extends GridCustom<TiposComprobantesConceptos> {

	private TiposComprobantesConceptosFiltro filter;
	private TipoComprobanteConceptoService service;

	public UITiposComprobantesConceptosGrid(TipoComprobanteConceptoService service, TiposComprobantesConceptosFiltro filter) {
	...
	protected void addColumns() {
		...
		addColumn(TiposComprobantesConceptos::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(TiposComprobantesConceptos::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Concepto")
			.setVisible(false);
	...
	*/
	...
	protected List<TiposComprobantesConceptos> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<TiposComprobantesConceptos> items = service.find(filter);
		...
		return new ArrayList<TiposComprobantesConceptos>();
	}

	protected boolean removeItemFromService(TiposComprobantesConceptos item) {
