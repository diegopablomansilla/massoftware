...
@PageTitle("Tickets")
@Route("Tickets")
public class UITicketsView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private TicketsFiltro lastFilter;
	private TicketsFiltro filter;
	private Binder<TicketsFiltro> binder;
	...
	// Grid
	private UITicketsGrid grid;

	public UITicketsView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new TicketsFiltro();
		binder = new Binder<>(TicketsFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		numeroFrom.setPlaceholder("Nº ticket desde ");
		...
		binder.forField(numeroFrom)
			...
			.bind(TicketsFiltro::getNumeroFrom, TicketsFiltro::setNumeroFrom);
		...
		numeroTo.setPlaceholder("Nº ticket hasta ");
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(TicketsFiltro::getNumeroTo, TicketsFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(TicketsFiltro::getNombre, TicketsFiltro::setNombre);
...
//		grid = new UITicketsGrid(AppCX.services().buildTicketService(), filter);
		grid = new UITicketsGrid(new TicketService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (TicketsFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
