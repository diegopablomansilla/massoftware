...
@Route("TiposComprobantesCopias")
public class UITiposComprobantesCopiasView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private TiposComprobantesCopiasFiltro lastFilter;
	private TiposComprobantesCopiasFiltro filter;
	private Binder<TiposComprobantesCopiasFiltro> binder;
	...
	// Grid
	private UITiposComprobantesCopiasGrid grid;

	public UITiposComprobantesCopiasView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new TiposComprobantesCopiasFiltro();
		binder = new Binder<>(TiposComprobantesCopiasFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		binder.forField(numeroFrom)
			...
			.bind(TiposComprobantesCopiasFiltro::getNumeroFrom, TiposComprobantesCopiasFiltro::setNumeroFrom);
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(TiposComprobantesCopiasFiltro::getNumeroTo, TiposComprobantesCopiasFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(TiposComprobantesCopiasFiltro::getNombre, TiposComprobantesCopiasFiltro::setNombre);
...
//		grid = new UITiposComprobantesCopiasGrid(AppCX.services().buildTipoComprobanteCopiaService(), filter);
		grid = new UITiposComprobantesCopiasGrid(new TipoComprobanteCopiaService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (TiposComprobantesCopiasFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
