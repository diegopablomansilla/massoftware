...
public class UITiposComprobantesCopiasGrid extends GridCustom<TiposComprobantesCopias> {

	private TiposComprobantesCopiasFiltro filter;
	private TipoComprobanteCopiaService service;

	public UITiposComprobantesCopiasGrid(TipoComprobanteCopiaService service, TiposComprobantesCopiasFiltro filter) {
		super(TiposComprobantesCopias.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(TiposComprobantesCopias::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(TiposComprobantesCopias::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<TiposComprobantesCopias> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<TiposComprobantesCopias> items = service.find(filter);
		...
		return new ArrayList<TiposComprobantesCopias>();
	}

	protected boolean removeItemFromService(TiposComprobantesCopias item) {
