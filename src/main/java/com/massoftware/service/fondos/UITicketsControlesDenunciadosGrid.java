...
public class UITicketsControlesDenunciadosGrid extends GridCustom<TicketsControlesDenunciados> {

	private TicketsControlesDenunciadosFiltro filter;
	private TicketControlDenunciadosService service;

	public UITicketsControlesDenunciadosGrid(TicketControlDenunciadosService service, TicketsControlesDenunciadosFiltro filter) {
		super(TicketsControlesDenunciados.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(TicketsControlesDenunciados::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(TicketsControlesDenunciados::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Control denunciados")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<TicketsControlesDenunciados> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<TicketsControlesDenunciados> items = service.find(filter);
		...
		return new ArrayList<TicketsControlesDenunciados>();
	}

	protected boolean removeItemFromService(TicketsControlesDenunciados item) {
