...
public class UITalonariosControladoresFizcalesGrid extends GridCustom<TalonariosControladoresFizcales> {

	private TalonariosControladoresFizcalesFiltro filter;
	private TalonarioControladorFizcalService service;

	public UITalonariosControladoresFizcalesGrid(TalonarioControladorFizcalService service, TalonariosControladoresFizcalesFiltro filter) {
		super(TalonariosControladoresFizcales.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(TalonariosControladoresFizcales::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(TalonariosControladoresFizcales::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Controlador fizcal")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<TalonariosControladoresFizcales> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<TalonariosControladoresFizcales> items = service.find(filter);
		...
		return new ArrayList<TalonariosControladoresFizcales>();
	}

	protected boolean removeItemFromService(TalonariosControladoresFizcales item) {
