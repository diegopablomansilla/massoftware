...
public class UITicketsModelosGrid extends GridCustom<TicketsModelos> {

	private TicketsModelosFiltro filter;
	private TicketModeloService service;

	public UITicketsModelosGrid(TicketModeloService service, TicketsModelosFiltro filter) {
	...
	protected void addColumns() {
		...
		addColumn(TicketsModelos::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(TicketsModelos::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Ticket modelo")
			.setVisible(false);
	...
	*/
	...
	protected List<TicketsModelos> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<TicketsModelos> items = service.find(filter);
	...
	protected boolean removeItemFromService(TicketsModelos item) {
