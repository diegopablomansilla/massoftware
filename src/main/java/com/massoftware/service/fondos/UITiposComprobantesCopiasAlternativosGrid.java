...
public class UITiposComprobantesCopiasAlternativosGrid extends GridCustom<TiposComprobantesCopiasAlternativos> {

	private TiposComprobantesCopiasAlternativosFiltro filter;
	private TipoComprobanteCopiaAlternativoService service;

	public UITiposComprobantesCopiasAlternativosGrid(TipoComprobanteCopiaAlternativoService service, TiposComprobantesCopiasAlternativosFiltro filter) {
		super(TiposComprobantesCopiasAlternativos.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(TiposComprobantesCopiasAlternativos::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(TiposComprobantesCopiasAlternativos::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Tipo de copias")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<TiposComprobantesCopiasAlternativos> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<TiposComprobantesCopiasAlternativos> items = service.find(filter);
		...
		return new ArrayList<TiposComprobantesCopiasAlternativos>();
	}

	protected boolean removeItemFromService(TiposComprobantesCopiasAlternativos item) {
