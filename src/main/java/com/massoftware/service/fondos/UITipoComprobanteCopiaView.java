...
@Route("TipoComprobanteCopia")
public class UITipoComprobanteCopiaView extends VerticalLayout implements HasUrlParameter<String> {

	private TipoComprobanteCopiaService service;		

	// Binder
	private TipoComprobanteCopia item;
	private Binder<TipoComprobanteCopia> binder;
	...
	public UITipoComprobanteCopiaView() throws Exception {
		service = new TipoComprobanteCopiaService();		
		buildBinder();
		buildForm();
		this.setHeightFull();
	}
	...
	private void buildBinder() {
		item = new TipoComprobanteCopia();
		binder = new Binder<>(TipoComprobanteCopia.class);
		binder.setBean(item);
	}
	...
	private void buildNumero() throws Exception {
		...
		binder.forField(numero)
			...
			.bind(TipoComprobanteCopia::getNumero, TipoComprobanteCopia::setNumero);
	}
	...
	private void buildNombre() throws Exception {
		...
		binder.forField(nombre)
			...
			.bind(TipoComprobanteCopia::getNombre, TipoComprobanteCopia::setNombre);
	}
	...
	public void search(String id) {
		...
		try {
			...
			if (binder.isValid()) {											
				...
				BinderValidationStatus<TipoComprobanteCopia> validate = binder.validate();
