...
@PageTitle("Comportamientos de comprobante")
@Route("ComportamientosComprobantes")
public class UIComportamientosComprobantesView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private ComportamientosComprobantesFiltro lastFilter;
	private ComportamientosComprobantesFiltro filter;
	private Binder<ComportamientosComprobantesFiltro> binder;
	...
	// Grid
	private UIComportamientosComprobantesGrid grid;

	public UIComportamientosComprobantesView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new ComportamientosComprobantesFiltro();
		binder = new Binder<>(ComportamientosComprobantesFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		//-------------------------------------------------------------------
		// Nº comportamiento (desde)
		numeroFrom = new NumberField();
		numeroFrom.setMin(1);
		numeroFrom.setMax(Integer.MAX_VALUE);
		numeroFrom.setPlaceholder("Nº comportamiento desde ");
		...
		binder.forField(numeroFrom)
			...
			.bind(ComportamientosComprobantesFiltro::getNumeroFrom, ComportamientosComprobantesFiltro::setNumeroFrom);
		...
		//-------------------------------------------------------------------
		// Nº comportamiento (hasta)
		numeroTo = new NumberField();
		numeroTo.setMin(1);
		numeroTo.setMax(Integer.MAX_VALUE);
		numeroTo.setPlaceholder("Nº comportamiento hasta ");
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(ComportamientosComprobantesFiltro::getNumeroTo, ComportamientosComprobantesFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(ComportamientosComprobantesFiltro::getNombre, ComportamientosComprobantesFiltro::setNombre);
...
//		grid = new UIComportamientosComprobantesGrid(AppCX.services().buildComportamientoComprobanteService(), filter);
		grid = new UIComportamientosComprobantesGrid(new ComportamientoComprobanteService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (ComportamientosComprobantesFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
