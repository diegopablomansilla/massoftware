...
@PageTitle("Controladores fizcales")
@Route("TalonariosControladoresFizcales")
public class UITalonariosControladoresFizcalesView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private TalonariosControladoresFizcalesFiltro lastFilter;
	private TalonariosControladoresFizcalesFiltro filter;
	private Binder<TalonariosControladoresFizcalesFiltro> binder;
	...
	// Grid
	private UITalonariosControladoresFizcalesGrid grid;

	public UITalonariosControladoresFizcalesView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new TalonariosControladoresFizcalesFiltro();
		binder = new Binder<>(TalonariosControladoresFizcalesFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		binder.forField(codigo)
			.bind(TalonariosControladoresFizcalesFiltro::getCodigo, TalonariosControladoresFizcalesFiltro::setCodigo);
		...
		binder.forField(nombre)
			.bind(TalonariosControladoresFizcalesFiltro::getNombre, TalonariosControladoresFizcalesFiltro::setNombre);
...
//		grid = new UITalonariosControladoresFizcalesGrid(AppCX.services().buildTalonarioControladorFizcalService(), filter);
		grid = new UITalonariosControladoresFizcalesGrid(new TalonarioControladorFizcalService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (TalonariosControladoresFizcalesFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
