...
public class UICuentasFondosTiposGrid extends GridCustom<CuentasFondosTipos> {

	private CuentasFondosTiposFiltro filter;
	private CuentaFondoTipoService service;

	public UICuentasFondosTiposGrid(CuentaFondoTipoService service, CuentasFondosTiposFiltro filter) {
		super(CuentasFondosTipos.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(CuentasFondosTipos::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(CuentasFondosTipos::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Tipo de cuenta fondo")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<CuentasFondosTipos> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<CuentasFondosTipos> items = service.find(filter);
		...
		return new ArrayList<CuentasFondosTipos>();
	}

	protected boolean removeItemFromService(CuentasFondosTipos item) {
