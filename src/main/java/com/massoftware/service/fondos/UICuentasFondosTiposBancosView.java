...
@PageTitle("Tipos de bancos")
@Route("CuentasFondosTiposBancos")
public class UICuentasFondosTiposBancosView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private CuentasFondosTiposBancosFiltro lastFilter;
	private CuentasFondosTiposBancosFiltro filter;
	private Binder<CuentasFondosTiposBancosFiltro> binder;
	...
	// Grid
	private UICuentasFondosTiposBancosGrid grid;

	public UICuentasFondosTiposBancosView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new CuentasFondosTiposBancosFiltro();
		binder = new Binder<>(CuentasFondosTiposBancosFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		binder.forField(numeroFrom)
			...
			.bind(CuentasFondosTiposBancosFiltro::getNumeroFrom, CuentasFondosTiposBancosFiltro::setNumeroFrom);
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(CuentasFondosTiposBancosFiltro::getNumeroTo, CuentasFondosTiposBancosFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(CuentasFondosTiposBancosFiltro::getNombre, CuentasFondosTiposBancosFiltro::setNombre);
...
//		grid = new UICuentasFondosTiposBancosGrid(AppCX.services().buildCuentaFondoTipoBancoService(), filter);
		grid = new UICuentasFondosTiposBancosGrid(new CuentaFondoTipoBancoService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (CuentasFondosTiposBancosFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
