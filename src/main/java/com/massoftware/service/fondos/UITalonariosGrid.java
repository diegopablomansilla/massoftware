...
public class UITalonariosGrid extends GridCustom<Talonarios> {

	private TalonariosFiltro filter;
	private TalonarioService service;

	public UITalonariosGrid(TalonarioService service, TalonariosFiltro filter) {
		super(Talonarios.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(Talonarios::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(Talonarios::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Talonario")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<Talonarios> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<Talonarios> items = service.find(filter);
	...
	protected boolean removeItemFromService(Talonarios item) {
