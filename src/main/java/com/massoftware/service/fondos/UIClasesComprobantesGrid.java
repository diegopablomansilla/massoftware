...
public class UIClasesComprobantesGrid extends GridCustom<ClasesComprobantes> {

	private ClasesComprobantesFiltro filter;
	private ClaseComprobanteService service;

	public UIClasesComprobantesGrid(ClaseComprobanteService service, ClasesComprobantesFiltro filter) {
		super(ClasesComprobantes.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(ClasesComprobantes::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(ClasesComprobantes::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Clase")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<ClasesComprobantes> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<ClasesComprobantes> items = service.find(filter);
		...
		return new ArrayList<ClasesComprobantes>();
	}

	protected boolean removeItemFromService(ClasesComprobantes item) {
