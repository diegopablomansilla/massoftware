...
@Route("TicketsControlesDenunciados")
public class UITicketsControlesDenunciadosView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private TicketsControlesDenunciadosFiltro lastFilter;
	private TicketsControlesDenunciadosFiltro filter;
	private Binder<TicketsControlesDenunciadosFiltro> binder;
	...
	// Grid
	private UITicketsControlesDenunciadosGrid grid;

	public UITicketsControlesDenunciadosView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new TicketsControlesDenunciadosFiltro();
		binder = new Binder<>(TicketsControlesDenunciadosFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		numeroFrom.setPlaceholder("Nº control desde ");
		...
		binder.forField(numeroFrom)
			...
			.bind(TicketsControlesDenunciadosFiltro::getNumeroFrom, TicketsControlesDenunciadosFiltro::setNumeroFrom);
		...
		numeroTo.setPlaceholder("Nº control hasta ");
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(TicketsControlesDenunciadosFiltro::getNumeroTo, TicketsControlesDenunciadosFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(TicketsControlesDenunciadosFiltro::getNombre, TicketsControlesDenunciadosFiltro::setNombre);
...
//		grid = new UITicketsControlesDenunciadosGrid(AppCX.services().buildTicketControlDenunciadosService(), filter);
		grid = new UITicketsControlesDenunciadosGrid(new TicketControlDenunciadosService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (TicketsControlesDenunciadosFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
