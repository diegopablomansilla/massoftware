...
@Route("CuentasFondosBancosCopias")
public class UICuentasFondosBancosCopiasView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private CuentasFondosBancosCopiasFiltro lastFilter;
	private CuentasFondosBancosCopiasFiltro filter;
	private Binder<CuentasFondosBancosCopiasFiltro> binder;
	...
	// Grid
	private UICuentasFondosBancosCopiasGrid grid;

	public UICuentasFondosBancosCopiasView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new CuentasFondosBancosCopiasFiltro();
		binder = new Binder<>(CuentasFondosBancosCopiasFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		binder.forField(numeroFrom)
			...
			.bind(CuentasFondosBancosCopiasFiltro::getNumeroFrom, CuentasFondosBancosCopiasFiltro::setNumeroFrom);
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(CuentasFondosBancosCopiasFiltro::getNumeroTo, CuentasFondosBancosCopiasFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(CuentasFondosBancosCopiasFiltro::getNombre, CuentasFondosBancosCopiasFiltro::setNombre);
...
//		grid = new UICuentasFondosBancosCopiasGrid(AppCX.services().buildCuentaFondoBancoCopiaService(), filter);
		grid = new UICuentasFondosBancosCopiasGrid(new CuentaFondoBancoCopiaService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (CuentasFondosBancosCopiasFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
