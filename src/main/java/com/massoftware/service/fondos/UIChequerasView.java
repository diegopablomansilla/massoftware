...
@PageTitle("Chequeras")
@Route("Chequeras")
public class UIChequerasView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private ChequerasFiltro lastFilter;
	private ChequerasFiltro filter;
	private Binder<ChequerasFiltro> binder;
	...
	// Grid
	private UIChequerasGrid grid;

	public UIChequerasView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new ChequerasFiltro();
		binder = new Binder<>(ChequerasFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		numeroFrom.setPlaceholder("Nº chequera desde ");
		...
		binder.forField(numeroFrom)
			...
			.bind(ChequerasFiltro::getNumeroFrom, ChequerasFiltro::setNumeroFrom);
		...
		numeroTo.setPlaceholder("Nº chequera hasta ");
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(ChequerasFiltro::getNumeroTo, ChequerasFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(ChequerasFiltro::getNombre, ChequerasFiltro::setNombre);
		...
		binder.forField(cuentaFondo)
			...
			.bind(ChequerasFiltro::getCuentaFondo, ChequerasFiltro::setCuentaFondo);
...
*/
		...
		filterRow1.add(newBTN, numeroFrom, numeroTo, nombre, cuentaFondo, findBTN);

		//-------------------------------------------------------------------
	}

	private void buildGrid() throws Exception {
//		grid = new UIChequerasGrid(AppCX.services().buildChequeraService(), filter);
		grid = new UIChequerasGrid(new ChequeraService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (ChequerasFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
