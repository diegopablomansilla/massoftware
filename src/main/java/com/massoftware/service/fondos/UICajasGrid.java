...
public class UICajasGrid extends GridCustom<Cajas> {

	private CajasFiltro filter;
	private CajaService service;

	public UICajasGrid(CajaService service, CajasFiltro filter) {
		super(Cajas.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(Cajas::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(Cajas::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Caja")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<Cajas> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<Cajas> items = service.find(filter);
	...
	protected boolean removeItemFromService(Cajas item) {
