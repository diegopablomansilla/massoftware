...
public class UITalonariosLetrasGrid extends GridCustom<TalonariosLetras> {

	private TalonariosLetrasFiltro filter;
	private TalonarioLetraService service;

	public UITalonariosLetrasGrid(TalonarioLetraService service, TalonariosLetrasFiltro filter) {
		super(TalonariosLetras.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(TalonariosLetras::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(TalonariosLetras::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Letra")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<TalonariosLetras> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<TalonariosLetras> items = service.find(filter);
		...
		return new ArrayList<TalonariosLetras>();
	}

	protected boolean removeItemFromService(TalonariosLetras item) {
