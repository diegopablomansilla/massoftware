...
@PageTitle("Módulos")
@Route("SeguridadModulos")
public class UISeguridadModulosView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private SeguridadModulosFiltro lastFilter;
	private SeguridadModulosFiltro filter;
	private Binder<SeguridadModulosFiltro> binder;
	...
	// Grid
	private UISeguridadModulosGrid grid;

	public UISeguridadModulosView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new SeguridadModulosFiltro();
		binder = new Binder<>(SeguridadModulosFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		binder.forField(numeroFrom)
			...
			.bind(SeguridadModulosFiltro::getNumeroFrom, SeguridadModulosFiltro::setNumeroFrom);
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(SeguridadModulosFiltro::getNumeroTo, SeguridadModulosFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(SeguridadModulosFiltro::getNombre, SeguridadModulosFiltro::setNombre);
...
//		grid = new UISeguridadModulosGrid(AppCX.services().buildSeguridadModuloService(), filter);
		grid = new UISeguridadModulosGrid(new SeguridadModuloService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (SeguridadModulosFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
