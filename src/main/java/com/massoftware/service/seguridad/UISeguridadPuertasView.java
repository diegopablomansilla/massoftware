...
public class UISeguridadPuertasView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private SeguridadPuertasFiltro lastFilter;
	private SeguridadPuertasFiltro filter;
	private Binder<SeguridadPuertasFiltro> binder;
	...
	// Grid
	private UISeguridadPuertasGrid grid;

	public UISeguridadPuertasView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new SeguridadPuertasFiltro();
		binder = new Binder<>(SeguridadPuertasFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		binder.forField(numeroFrom)
			...
			.bind(SeguridadPuertasFiltro::getNumeroFrom, SeguridadPuertasFiltro::setNumeroFrom);
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(SeguridadPuertasFiltro::getNumeroTo, SeguridadPuertasFiltro::setNumeroTo);
