...
public class UIUsuariosGrid extends GridCustom<Usuarios> {

	private UsuariosFiltro filter;
	private UsuarioService service;

	public UIUsuariosGrid(UsuarioService service, UsuariosFiltro filter) {
	...
	protected void addColumns() {
		...
		addColumn(Usuarios::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(Usuarios::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Usuario")
			.setVisible(false);
	...
	*/
	...
	protected List<Usuarios> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<Usuarios> items = service.find(filter);
	...
	protected boolean removeItemFromService(Usuarios item) {
