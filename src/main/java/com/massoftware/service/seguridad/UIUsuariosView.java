...
@PageTitle("Usuarios")
@Route("Usuarios")
public class UIUsuariosView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private UsuariosFiltro lastFilter;
	private UsuariosFiltro filter;
	private Binder<UsuariosFiltro> binder;
	...
	// Grid
	private UIUsuariosGrid grid;

	public UIUsuariosView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new UsuariosFiltro();
		binder = new Binder<>(UsuariosFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		numeroFrom.setPlaceholder("Nº usuario desde ");
		...
		binder.forField(numeroFrom)
			...
			.bind(UsuariosFiltro::getNumeroFrom, UsuariosFiltro::setNumeroFrom);
		...
		numeroTo.setPlaceholder("Nº usuario hasta ");
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(UsuariosFiltro::getNumeroTo, UsuariosFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(UsuariosFiltro::getNombre, UsuariosFiltro::setNombre);
...
//		grid = new UIUsuariosGrid(AppCX.services().buildUsuarioService(), filter);
		grid = new UIUsuariosGrid(new UsuarioService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (UsuariosFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
