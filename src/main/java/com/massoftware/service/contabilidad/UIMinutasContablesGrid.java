...
public class UIMinutasContablesGrid extends GridCustom<MinutasContables> {

	private MinutasContablesFiltro filter;
	private MinutaContableService service;

	public UIMinutasContablesGrid(MinutaContableService service, MinutasContablesFiltro filter) {
		super(MinutasContables.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(MinutasContables::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(MinutasContables::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Minuta contable")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<MinutasContables> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<MinutasContables> items = service.find(filter);
		...
		return new ArrayList<MinutasContables>();
	}

	protected boolean removeItemFromService(MinutasContables item) {
