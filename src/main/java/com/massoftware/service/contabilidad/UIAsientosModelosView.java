...
@Route("AsientosModelos")
public class UIAsientosModelosView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private AsientosModelosFiltro lastFilter;
	private AsientosModelosFiltro filter;
	private Binder<AsientosModelosFiltro> binder;
	...
	// Grid
	private UIAsientosModelosGrid grid;

	public UIAsientosModelosView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new AsientosModelosFiltro();
		binder = new Binder<>(AsientosModelosFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		numeroFrom.setPlaceholder("Nº asientodesde ");
		...
		binder.forField(numeroFrom)
			...
			.bind(AsientosModelosFiltro::getNumeroFrom, AsientosModelosFiltro::setNumeroFrom);
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(AsientosModelosFiltro::getNumeroTo, AsientosModelosFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(AsientosModelosFiltro::getNombre, AsientosModelosFiltro::setNombre);
		...
		binder.forField(ejercicioContable)
			...
			.bind(AsientosModelosFiltro::getEjercicioContable, AsientosModelosFiltro::setEjercicioContable);
...
//		grid = new UIAsientosModelosGrid(AppCX.services().buildAsientoModeloService(), filter);
		grid = new UIAsientosModelosGrid(new AsientoModeloService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (AsientosModelosFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
