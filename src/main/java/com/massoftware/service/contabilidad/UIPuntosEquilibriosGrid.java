...
public class UIPuntosEquilibriosGrid extends GridCustom<PuntosEquilibrios> {

	private PuntosEquilibriosFiltro filter;
	private PuntoEquilibrioService service;

	public UIPuntosEquilibriosGrid(PuntoEquilibrioService service, PuntosEquilibriosFiltro filter) {
		super(PuntosEquilibrios.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(PuntosEquilibrios::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(PuntosEquilibrios::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Punto equilibrio")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<PuntosEquilibrios> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			if (filter.getEjercicioContable() == null || filter.getEjercicioContable().toString().trim().isEmpty()) {
				return new ArrayList<PuntosEquilibrios>();
			}
			...
			List<PuntosEquilibrios> items = service.find(filter);
	...
	protected boolean removeItemFromService(PuntosEquilibrios item) {
