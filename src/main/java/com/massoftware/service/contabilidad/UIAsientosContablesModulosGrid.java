...
public class UIAsientosContablesModulosGrid extends GridCustom<AsientosContablesModulos> {

	private AsientosContablesModulosFiltro filter;
	private AsientoContableModuloService service;

	public UIAsientosContablesModulosGrid(AsientoContableModuloService service, AsientosContablesModulosFiltro filter) {
		super(AsientosContablesModulos.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(AsientosContablesModulos::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(AsientosContablesModulos::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Módulo")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<AsientosContablesModulos> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<AsientosContablesModulos> items = service.find(filter);
		...
		return new ArrayList<AsientosContablesModulos>();
	}

	protected boolean removeItemFromService(AsientosContablesModulos item) {
