...
public class UIEjerciciosContablesView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private EjerciciosContablesFiltro lastFilter;
	private EjerciciosContablesFiltro filter;
	private Binder<EjerciciosContablesFiltro> binder;
	...
	// Grid
	private UIEjerciciosContablesGrid grid;

	public UIEjerciciosContablesView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new EjerciciosContablesFiltro();
		binder = new Binder<>(EjerciciosContablesFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		binder.forField(numeroFrom)
			...
			.bind(EjerciciosContablesFiltro::getNumeroFrom, EjerciciosContablesFiltro::setNumeroFrom);
		...
		numeroTo.setPlaceholder("Nº ejercicio hasta ");
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(EjerciciosContablesFiltro::getNumeroTo, EjerciciosContablesFiltro::setNumeroTo);
