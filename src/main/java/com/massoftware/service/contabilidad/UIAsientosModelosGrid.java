...
public class UIAsientosModelosGrid extends GridCustom<AsientosModelos> {

	private AsientosModelosFiltro filter;
	private AsientoModeloService service;

	public UIAsientosModelosGrid(AsientoModeloService service, AsientosModelosFiltro filter) {
	...
	protected void addColumns() {
		...
		addColumn(AsientosModelos::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(AsientosModelos::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Asiento modelo")
			.setVisible(false);
	...
	*/
	...
	protected List<AsientosModelos> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<AsientosModelos> items = service.find(filter);
	...
	protected boolean removeItemFromService(AsientosModelos item) {
