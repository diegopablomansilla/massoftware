...
@PageTitle("Minutas contables")
@Route("MinutasContables")
public class UIMinutasContablesView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private MinutasContablesFiltro lastFilter;
	private MinutasContablesFiltro filter;
	private Binder<MinutasContablesFiltro> binder;
	...
	// Grid
	private UIMinutasContablesGrid grid;

	public UIMinutasContablesView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new MinutasContablesFiltro();
		binder = new Binder<>(MinutasContablesFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		numeroFrom.setPlaceholder("Nº minuta desde ");
		...
		binder.forField(numeroFrom)
			...
			.bind(MinutasContablesFiltro::getNumeroFrom, MinutasContablesFiltro::setNumeroFrom);
		...
		numeroTo.setPlaceholder("Nº minuta hasta ");
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(MinutasContablesFiltro::getNumeroTo, MinutasContablesFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(MinutasContablesFiltro::getNombre, MinutasContablesFiltro::setNombre);
...
//		grid = new UIMinutasContablesGrid(AppCX.services().buildMinutaContableService(), filter);
		grid = new UIMinutasContablesGrid(new MinutaContableService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (MinutasContablesFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
