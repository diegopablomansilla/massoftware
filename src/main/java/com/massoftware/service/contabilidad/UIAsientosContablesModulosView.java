...
@PageTitle("Módulos")
@Route("AsientosContablesModulos")
public class UIAsientosContablesModulosView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private AsientosContablesModulosFiltro lastFilter;
	private AsientosContablesModulosFiltro filter;
	private Binder<AsientosContablesModulosFiltro> binder;
	...
	// Grid
	private UIAsientosContablesModulosGrid grid;

	public UIAsientosContablesModulosView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new AsientosContablesModulosFiltro();
		binder = new Binder<>(AsientosContablesModulosFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		numeroFrom.setPlaceholder("Nº módulo desde ");
		...
		binder.forField(numeroFrom)
			...
			.bind(AsientosContablesModulosFiltro::getNumeroFrom, AsientosContablesModulosFiltro::setNumeroFrom);
		...
		numeroTo.setPlaceholder("Nº módulo hasta ");
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(AsientosContablesModulosFiltro::getNumeroTo, AsientosContablesModulosFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(AsientosContablesModulosFiltro::getNombre, AsientosContablesModulosFiltro::setNombre);
...
//		grid = new UIAsientosContablesModulosGrid(AppCX.services().buildAsientoContableModuloService(), filter);
		grid = new UIAsientosContablesModulosGrid(new AsientoContableModuloService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (AsientosContablesModulosFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
