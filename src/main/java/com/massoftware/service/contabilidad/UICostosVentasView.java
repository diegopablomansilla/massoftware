...
@PageTitle("Costos de venta")
@Route("CostosVentas")
public class UICostosVentasView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private CostosVentasFiltro lastFilter;
	private CostosVentasFiltro filter;
	private Binder<CostosVentasFiltro> binder;
	...
	// Grid
	private UICostosVentasGrid grid;

	public UICostosVentasView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new CostosVentasFiltro();
		binder = new Binder<>(CostosVentasFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		binder.forField(numeroFrom)
			...
			.bind(CostosVentasFiltro::getNumeroFrom, CostosVentasFiltro::setNumeroFrom);
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(CostosVentasFiltro::getNumeroTo, CostosVentasFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(CostosVentasFiltro::getNombre, CostosVentasFiltro::setNombre);
...
//		grid = new UICostosVentasGrid(AppCX.services().buildCostoVentaService(), filter);
		grid = new UICostosVentasGrid(new CostoVentaService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (CostosVentasFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
