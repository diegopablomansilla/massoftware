...
public class UICuentasContablesEstadosGrid extends GridCustom<CuentasContablesEstados> {

	private CuentasContablesEstadosFiltro filter;
	private CuentaContableEstadoService service;

	public UICuentasContablesEstadosGrid(CuentaContableEstadoService service, CuentasContablesEstadosFiltro filter) {
		super(CuentasContablesEstados.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(CuentasContablesEstados::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(CuentasContablesEstados::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Estado")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<CuentasContablesEstados> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<CuentasContablesEstados> items = service.find(filter);
		...
		return new ArrayList<CuentasContablesEstados>();
	}

	protected boolean removeItemFromService(CuentasContablesEstados item) {
