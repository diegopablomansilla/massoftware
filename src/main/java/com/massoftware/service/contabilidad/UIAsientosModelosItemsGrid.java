...
public class UIAsientosModelosItemsGrid extends GridCustom<AsientosModelosItems> {

	private AsientosModelosItemsFiltro filter;
	private AsientoModeloItemService service;

	public UIAsientosModelosItemsGrid(AsientoModeloItemService service, AsientosModelosItemsFiltro filter) {
		super(AsientosModelosItems.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(AsientosModelosItems::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(AsientosModelosItems::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Item de asiento modelo")
			.setVisible(false);
	...
	*/
	...
	protected Integer countFromService() {
		...
		try {
			...
			if (filter.getAsientoModelo() == null || filter.getAsientoModelo().toString().trim().isEmpty()) {
				return 0;
			}
	...
	protected List<AsientosModelosItems> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			if (filter.getAsientoModelo() == null || filter.getAsientoModelo().toString().trim().isEmpty()) {
				return new ArrayList<AsientosModelosItems>();
			}
			...
			List<AsientosModelosItems> items = service.find(filter);
	...
	protected boolean removeItemFromService(AsientosModelosItems item) {
