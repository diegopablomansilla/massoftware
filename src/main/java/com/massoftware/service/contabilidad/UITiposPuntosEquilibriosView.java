...
@PageTitle("Tipos de puntos equilibrio")
@Route("TiposPuntosEquilibrios")
public class UITiposPuntosEquilibriosView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private TiposPuntosEquilibriosFiltro lastFilter;
	private TiposPuntosEquilibriosFiltro filter;
	private Binder<TiposPuntosEquilibriosFiltro> binder;
	...
	// Grid
	private UITiposPuntosEquilibriosGrid grid;

	public UITiposPuntosEquilibriosView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new TiposPuntosEquilibriosFiltro();
		binder = new Binder<>(TiposPuntosEquilibriosFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		binder.forField(numeroFrom)
			...
			.bind(TiposPuntosEquilibriosFiltro::getNumeroFrom, TiposPuntosEquilibriosFiltro::setNumeroFrom);
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(TiposPuntosEquilibriosFiltro::getNumeroTo, TiposPuntosEquilibriosFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(TiposPuntosEquilibriosFiltro::getNombre, TiposPuntosEquilibriosFiltro::setNombre);
...
//		grid = new UITiposPuntosEquilibriosGrid(AppCX.services().buildTipoPuntoEquilibrioService(), filter);
		grid = new UITiposPuntosEquilibriosGrid(new TipoPuntoEquilibrioService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (TiposPuntosEquilibriosFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
