...
@PageTitle("Motivos de nota credito")
@Route("NotasCreditoMotivo")
public class UINotasCreditoMotivoView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private NotasCreditoMotivoFiltro lastFilter;
	private NotasCreditoMotivoFiltro filter;
	private Binder<NotasCreditoMotivoFiltro> binder;
	...
	// Grid
	private UINotasCreditoMotivoGrid grid;

	public UINotasCreditoMotivoView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new NotasCreditoMotivoFiltro();
		binder = new Binder<>(NotasCreditoMotivoFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		binder.forField(numeroFrom)
			...
			.bind(NotasCreditoMotivoFiltro::getNumeroFrom, NotasCreditoMotivoFiltro::setNumeroFrom);
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(NotasCreditoMotivoFiltro::getNumeroTo, NotasCreditoMotivoFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(NotasCreditoMotivoFiltro::getNombre, NotasCreditoMotivoFiltro::setNombre);
...
//		grid = new UINotasCreditoMotivoGrid(AppCX.services().buildNotaCreditoMotivoService(), filter);
		grid = new UINotasCreditoMotivoGrid(new NotaCreditoMotivoService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (NotasCreditoMotivoFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
