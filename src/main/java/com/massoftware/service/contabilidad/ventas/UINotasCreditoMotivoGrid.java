...
public class UINotasCreditoMotivoGrid extends GridCustom<NotasCreditoMotivo> {

	private NotasCreditoMotivoFiltro filter;
	private NotaCreditoMotivoService service;

	public UINotasCreditoMotivoGrid(NotaCreditoMotivoService service, NotasCreditoMotivoFiltro filter) {
		super(NotasCreditoMotivo.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(NotasCreditoMotivo::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(NotasCreditoMotivo::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Motivo de nota credito")
			.setVisible(false);
		...
		addColumn(NotasCreditoMotivo::getNumero, "numero")
			.setKey("numero")
			.setResizable(true)
			.setSortProperty("2")
			.setHeader("Nº motivo");

		addColumn(NotasCreditoMotivo::getNombre, "nombre")
			.setKey("nombre")
			.setResizable(true)
			.setSortProperty("3")
			.setHeader("Nombre");
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<NotasCreditoMotivo> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<NotasCreditoMotivo> items = service.find(filter);
		...
		return new ArrayList<NotasCreditoMotivo>();
	}

	protected boolean removeItemFromService(NotasCreditoMotivo item) {
