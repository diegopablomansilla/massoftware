...
public class UICentrosCostosContablesView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private CentrosCostosContablesFiltro lastFilter;
	private CentrosCostosContablesFiltro filter;
	private Binder<CentrosCostosContablesFiltro> binder;
	...
	// Grid
	private UICentrosCostosContablesGrid grid;

	public UICentrosCostosContablesView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new CentrosCostosContablesFiltro();
		binder = new Binder<>(CentrosCostosContablesFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		binder.forField(numeroFrom)
			...
			.bind(CentrosCostosContablesFiltro::getNumeroFrom, CentrosCostosContablesFiltro::setNumeroFrom);
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(CentrosCostosContablesFiltro::getNumeroTo, CentrosCostosContablesFiltro::setNumeroTo);
