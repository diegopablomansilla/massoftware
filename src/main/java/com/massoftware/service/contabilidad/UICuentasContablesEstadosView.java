...
@PageTitle("Estados")
@Route("CuentasContablesEstados")
public class UICuentasContablesEstadosView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private CuentasContablesEstadosFiltro lastFilter;
	private CuentasContablesEstadosFiltro filter;
	private Binder<CuentasContablesEstadosFiltro> binder;
	...
	// Grid
	private UICuentasContablesEstadosGrid grid;

	public UICuentasContablesEstadosView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new CuentasContablesEstadosFiltro();
		binder = new Binder<>(CuentasContablesEstadosFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		binder.forField(numeroFrom)
			...
			.bind(CuentasContablesEstadosFiltro::getNumeroFrom, CuentasContablesEstadosFiltro::setNumeroFrom);
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(CuentasContablesEstadosFiltro::getNumeroTo, CuentasContablesEstadosFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(CuentasContablesEstadosFiltro::getNombre, CuentasContablesEstadosFiltro::setNombre);
...
//		grid = new UICuentasContablesEstadosGrid(AppCX.services().buildCuentaContableEstadoService(), filter);
		grid = new UICuentasContablesEstadosGrid(new CuentaContableEstadoService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (CuentasContablesEstadosFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
