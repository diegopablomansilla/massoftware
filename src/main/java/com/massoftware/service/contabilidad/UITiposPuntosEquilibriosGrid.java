...
public class UITiposPuntosEquilibriosGrid extends GridCustom<TiposPuntosEquilibrios> {

	private TiposPuntosEquilibriosFiltro filter;
	private TipoPuntoEquilibrioService service;

	public UITiposPuntosEquilibriosGrid(TipoPuntoEquilibrioService service, TiposPuntosEquilibriosFiltro filter) {
		super(TiposPuntosEquilibrios.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(TiposPuntosEquilibrios::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(TiposPuntosEquilibrios::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<TiposPuntosEquilibrios> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<TiposPuntosEquilibrios> items = service.find(filter);
		...
		return new ArrayList<TiposPuntosEquilibrios>();
	}

	protected boolean removeItemFromService(TiposPuntosEquilibrios item) {
