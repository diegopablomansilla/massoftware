...
@Route("MonedasCotizaciones")
public class UIMonedasCotizacionesView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private MonedasCotizacionesFiltro lastFilter;
	private MonedasCotizacionesFiltro filter;
	private Binder<MonedasCotizacionesFiltro> binder;
	...
	private ComboBox<Monedas> moneda;

	private Button newBTN;
	private Button findBTN;

	// Grid
	private UIMonedasCotizacionesGrid grid;

	public UIMonedasCotizacionesView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new MonedasCotizacionesFiltro();
		binder = new Binder<>(MonedasCotizacionesFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		if(monedaItems.size() > 0){
			moneda.setValue(monedaItems.get(0));
		}
		moneda.addValueChangeListener(event -> {
			search();
		});
		moneda.addBlurListener(event -> {
			search();
		});
...
//		grid = new UIMonedasCotizacionesGrid(AppCX.services().buildMonedaCotizacionService(), filter);
		grid = new UIMonedasCotizacionesGrid(new MonedaCotizacionService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (MonedasCotizacionesFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
