...
@PageTitle("Monedas")
@Route("Monedas")
public class UIMonedasView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private MonedasFiltro lastFilter;
	private MonedasFiltro filter;
	private Binder<MonedasFiltro> binder;
	...
	// Grid
	private UIMonedasGrid grid;

	public UIMonedasView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();
	...
	private void buildBinder() {
		filter = new MonedasFiltro();
		binder = new Binder<>(MonedasFiltro.class);
	...
	private void buildFilterRows() {
		...
		binder.forField(numeroFrom)
			...
			.bind(MonedasFiltro::getNumeroFrom, MonedasFiltro::setNumeroFrom);
		...
		numeroTo.setPlaceholder("Nº moneda hasta ");
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(MonedasFiltro::getNumeroTo, MonedasFiltro::setNumeroTo);
		...
		binder.bind(nombre, MonedasFiltro::getNombre, MonedasFiltro::setNombre);
		...
		binder.bind(abreviatura, MonedasFiltro::getAbreviatura, MonedasFiltro::setAbreviatura);
...
*/
	...
	private void buildGrid() throws Exception {
		grid = new UIMonedasGrid(AppCX.services().buildMonedaService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (MonedasFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
