...
public class UIMonedasGrid extends GridCustom<Monedas> {

	private MonedasFiltro filter;
	private MonedaService service;

	public UIMonedasGrid(MonedaService service, MonedasFiltro filter) {
		super(Monedas.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(Monedas::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(Monedas::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Moneda")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<Monedas> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<Monedas> items = service.find(filter);
	...
	protected boolean removeItemFromService(Monedas item) {
