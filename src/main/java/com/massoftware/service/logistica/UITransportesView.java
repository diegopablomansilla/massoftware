...
@Route("Transportes")
public class UITransportesView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private TransportesFiltro lastFilter;
	private TransportesFiltro filter;
	private Binder<TransportesFiltro> binder;
	...
	// Grid
	private UITransportesGrid grid;

	public UITransportesView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new TransportesFiltro();
		binder = new Binder<>(TransportesFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		numeroFrom.setPlaceholder("Nº transporte desde ");
		...
		binder.forField(numeroFrom)
			...
			.bind(TransportesFiltro::getNumeroFrom, TransportesFiltro::setNumeroFrom);
		...
		numeroTo.setPlaceholder("Nº transporte hasta ");
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(TransportesFiltro::getNumeroTo, TransportesFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(TransportesFiltro::getNombre, TransportesFiltro::setNombre);
...
//		grid = new UITransportesGrid(AppCX.services().buildTransporteService(), filter);
		grid = new UITransportesGrid(new TransporteService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (TransportesFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
