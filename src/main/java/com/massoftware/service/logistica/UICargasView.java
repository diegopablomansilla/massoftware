...
@PageTitle("Cargas")
@Route("Cargas")
public class UICargasView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private CargasFiltro lastFilter;
	private CargasFiltro filter;
	private Binder<CargasFiltro> binder;
	...
	// Grid
	private UICargasGrid grid;

	public UICargasView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();
	...
	private void buildBinder() {
		filter = new CargasFiltro();
		binder = new Binder<>(CargasFiltro.class);
	...
	private void buildFilterRows() {
		...
		binder.forField(numeroFrom)
			...
			.bind(CargasFiltro::getNumeroFrom, CargasFiltro::setNumeroFrom);
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(CargasFiltro::getNumeroTo, CargasFiltro::setNumeroTo);
		...
		binder.bind(nombre, CargasFiltro::getNombre, CargasFiltro::setNombre);
...
*/
	...
	private void buildGrid() throws Exception {
		grid = new UICargasGrid(AppCX.services().buildCargaService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (CargasFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
