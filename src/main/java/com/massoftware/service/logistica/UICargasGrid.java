...
public class UICargasGrid extends GridCustom<Cargas> {
	...
	public UICargasGrid(CargaService service, CargasFiltro filter) {
		super(Cargas.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(Cargas::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(Cargas::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Carga")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<Cargas> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<Cargas> items = service.find(filter);
	...
	protected boolean removeItemFromService(Cargas item) {
