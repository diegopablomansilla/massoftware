...
public class UITiposClientesGrid extends GridCustom<TiposClientes> {

	private TiposClientesFiltro filter;
	private TipoClienteService service;

	public UITiposClientesGrid(TipoClienteService service, TiposClientesFiltro filter) {
		super(TiposClientes.class, true, true, true, true);
		this.filter = filter;
	...
	protected void addColumns() {
		...
		addColumn(TiposClientes::getId, "id")
			.setKey("id")
			.setSortProperty("1")
			.setHeader("ID")
			.setVisible(false);

		addColumn(TiposClientes::toString, "toString")
			.setKey("toString")
			.setSortProperty("2")
			.setHeader("Tipo de cliente")
			.setVisible(false);
...
//		addColumn(Bancos::getCuit, "cuit").setKey("cuit").setResizable(true).setSortProperty("4").setHeader("CUIT");
	...
	*/
	...
	protected List<TiposClientes> findFromService(int offset, int limit, Integer orderBy, Boolean orderByDesc) {
		...
		try {
			...
			List<TiposClientes> items = service.find(filter);
		...
		return new ArrayList<TiposClientes>();
	}

	protected boolean removeItemFromService(TiposClientes item) {
