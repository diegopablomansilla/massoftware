...
public class UIMotivosComentarioView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private MotivosComentarioFiltro lastFilter;
	private MotivosComentarioFiltro filter;
	private Binder<MotivosComentarioFiltro> binder;
	...
	// Grid
	private UIMotivosComentarioGrid grid;

	public UIMotivosComentarioView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new MotivosComentarioFiltro();
		binder = new Binder<>(MotivosComentarioFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		binder.forField(numeroFrom)
			...
			.bind(MotivosComentarioFiltro::getNumeroFrom, MotivosComentarioFiltro::setNumeroFrom);
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(MotivosComentarioFiltro::getNumeroTo, MotivosComentarioFiltro::setNumeroTo);
