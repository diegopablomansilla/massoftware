...
@PageTitle("Clasificaciones de clientes")
@Route("ClasificacionesClientes")
public class UIClasificacionesClientesView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private ClasificacionesClientesFiltro lastFilter;
	private ClasificacionesClientesFiltro filter;
	private Binder<ClasificacionesClientesFiltro> binder;
	...
	// Grid
	private UIClasificacionesClientesGrid grid;

	public UIClasificacionesClientesView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new ClasificacionesClientesFiltro();
		binder = new Binder<>(ClasificacionesClientesFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		numeroFrom.setPlaceholder("Nº clasificación desde ");
		...
		binder.forField(numeroFrom)
			...
			.bind(ClasificacionesClientesFiltro::getNumeroFrom, ClasificacionesClientesFiltro::setNumeroFrom);
		...
		numeroTo.setPlaceholder("Nº clasificación hasta ");
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(ClasificacionesClientesFiltro::getNumeroTo, ClasificacionesClientesFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(ClasificacionesClientesFiltro::getNombre, ClasificacionesClientesFiltro::setNombre);
...
//		grid = new UIClasificacionesClientesGrid(AppCX.services().buildClasificacionClienteService(), filter);
		grid = new UIClasificacionesClientesGrid(new ClasificacionClienteService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (ClasificacionesClientesFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
