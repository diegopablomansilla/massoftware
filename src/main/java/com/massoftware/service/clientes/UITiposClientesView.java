...
@Route("TiposClientes")
public class UITiposClientesView extends VerticalLayout {
...
// botonera no ordenable no movible
	...
	// Binder
	private TiposClientesFiltro lastFilter;
	private TiposClientesFiltro filter;
	private Binder<TiposClientesFiltro> binder;
	...
	// Grid
	private UITiposClientesGrid grid;

	public UITiposClientesView() throws Exception {
		buildBinder();
		buildFilterRows();
		buildGrid();
		this.setHeightFull();		
	...
	private void buildBinder() {
		filter = new TiposClientesFiltro();
		binder = new Binder<>(TiposClientesFiltro.class);
	...
	private void buildFilterRows() throws Exception {
		...
		//-------------------------------------------------------------------
		// Nº tipo de cliente (desde)
		numeroFrom = new NumberField();
		numeroFrom.setMin(1);
		numeroFrom.setMax(Integer.MAX_VALUE);
		numeroFrom.setPlaceholder("Nº tipo de cliente desde ");
		...
		binder.forField(numeroFrom)
			...
			.bind(TiposClientesFiltro::getNumeroFrom, TiposClientesFiltro::setNumeroFrom);
		...
		//-------------------------------------------------------------------
		// Nº tipo de cliente (hasta)
		numeroTo = new NumberField();
		numeroTo.setMin(1);
		numeroTo.setMax(Integer.MAX_VALUE);
		numeroTo.setPlaceholder("Nº tipo de cliente hasta ");
		...
		binder.forField(numeroTo)
			...
			.withValidator(value -> (value != null) ? value >= 1 : true, "El valor tiene que ser >= 1")
			.withValidator(value -> (value != null) ? value <= Integer.MAX_VALUE : true,"El valor tiene que ser <= " + Integer.MAX_VALUE)
			.bind(TiposClientesFiltro::getNumeroTo, TiposClientesFiltro::setNumeroTo);
		...
		binder.forField(nombre)
			.bind(TiposClientesFiltro::getNombre, TiposClientesFiltro::setNombre);
...
//		grid = new UITiposClientesGrid(AppCX.services().buildTipoClienteService(), filter);
		grid = new UITiposClientesGrid(new TipoClienteService(), filter);
...
//		grid.setHeightFull();
	...
	private void search() {
		...
		if (this.filter.equals(this.lastFilter) == false) {
			this.lastFilter = (TiposClientesFiltro) this.filter.clone();
			if (binder.isValid()) {
				grid.refreshAll();
			} else {
